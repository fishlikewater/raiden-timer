 ...
 */
package com.fishlikewater.fishlikewater.raiden.timer.core.utils;
...
public class CronSequenceGenerator {
    ...
    private void doNext(Calendar calendar, int dot) {
        ...
        if (month != updateMonth) {
            if (calendar.get(Calendar.YEAR) - dot > 4) {
                throw new IllegalArgumentException(STR."Invalid cron expression \"\{this.expression}\" led to runaway search for next trigger");
            }
            doNext(calendar, dot);
        }
    ...
    private int findNextDay(Calendar calendar, BitSet daysOfMonth, int dayOfMonth, BitSet daysOfWeek, int dayOfWeek, List<Integer> resets) {
        ...
        if (count >= max) {
            throw new IllegalArgumentException(STR."Overflow in day for expression \"\{this.expression}\"");
        }
        return dayOfMonth;
    }

    /**
     * Search the bits provided for the next set a bit after the value provided,
    ...
    private int[] getRange(String field, int min, int max) {
        ...
        if (!field.contains("-")) {
            ...
            String[] split = StringUtils.delimitedListToStringArray(field, "-");
            if (split.length > 2) {
                throw new IllegalArgumentException(STR."Range has more than two fields: '\{field}' in expression \"\{this.expression}\"");
            }
            result[0] = Integer.parseInt(split[0]);
            result[1] = Integer.parseInt(split[1]);
        }
        if (result[0] >= max || result[1] >= max) {
            throw new IllegalArgumentException(STR."Range exceeds maximum (\{max}): '\{field}' in expression \"\{this.expression}\"");
        }
        if (result[0] < min || result[1] < min) {
            throw new IllegalArgumentException(STR."Range less than minimum (\{min}): '\{field}' in expression \"\{this.expression}\"");
        }
        if (result[0] > result[1]) {
            throw new IllegalArgumentException(STR."Invalid inverted range: '\{field}' in expression \"\{this.expression}\"");
        }
        return result;
    }
    ...
    public String toString() {
        return STR."\{getClass().getSimpleName()}: \{this.expression}";
    }
